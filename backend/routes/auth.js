...
router.post("/login", async (req, res) => {
  ...
  const { rows } = await pool.query("SELECT id,email,pass_hash,name,role,active FROM users WHERE email=$1", [email]);
  const u = rows[0];
...
router.get("/me", (req, res) => {
  try {
    const t = req.cookies?.token; if (!t) return res.status(401).json({ error: "unauthenticated" });
